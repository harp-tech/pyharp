...
df = data.parse_to_dataframe(
    AnalogData, raw, time_index=True, keep_type=False, decode_enums=True
)
```

`time_index` decides the index: `True`, the default, gives the Harp time in float seconds named `"Time"`, a datetime such as `REFERENCE_EPOCH` gives an absolute `DatetimeIndex`, and `False` gives a `RangeIndex`. Enum fields decode to `pd.Categorical`, and `decode_enums=False` keeps raw codes.
